...
describe('LatestPostList tests', () => {
  afterEach(() => {
    cleanup();
  });

  it('Should show a latest post list', async () => {
    ...
    const { getByText } = render(<LatestPostsList />);

    expect(getByText('Post Title')).toBeDefined();
  });

  it('Should show a latest post list with 3 items', async () => {
    useSelector.mockImplementation((cb) =>
      cb({
        post: {
          data: [
    ...
    const { container } = render(<LatestPostsList />);

    const liLength = container.querySelectorAll('li').length;
